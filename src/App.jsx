...
function App() {
  const [view, setView] = useState(() => {
    ...
    if (decodedHash.startsWith("#/ubb")) return "ubb";
  ...
  useEffect(() => {
    const handleHashChange = () => {
      ...
      if (decodedHash.startsWith("#/unitbv")) {
        ...
        setView("utcn");
      } else if (decodedHash.startsWith("#/ubb")) {
        setView("ubb");
  ...
  const navigateTo = (newView) => {
    if (newView === "unitbv") {
      ...
      window.location.hash = `/ubb-${encodeURIComponent("Facultatea de Matematică și Informatică")}`;
    } else {
  ...
  if (view === "ubb") {
    return <UbbCluj onBack={() => navigateTo("home")} />;
  }
    ...
    <div className="w-full h-screen overflow-y-auto bg-bg text-text transition-colors duration-200">
     <div className="min-h-screen flex flex-col items-center justify-center p-6 md:p-12 gap-8">
      ...
      <div className="flex flex-col gap-6 w-full items-center max-w-[538px] mt-2">
        ...
        {/* Card UBB */}
        <Card
          onClick={() => navigateTo("ubb")}
          className="relative flex flex-col justify-between overflow-hidden cursor-pointer rounded-2xl shadow-md hover:shadow-2xl transition-all w-full h-[295px] p-6 group"
        >
          {/* Background Image */}
          <img
            src="https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&w=1080&q=80"
            alt="Universitatea Babeș-Bolyai"
            className="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
          />
          {/* Dark Overlay */}
          <div className="absolute inset-0 bg-gradient-to-t from-black/95 via-black/60 to-black/30 z-10" />
          ...
          <div className="relative z-20 flex flex-col justify-between h-full text-white">
            <div className="flex justify-between items-start">
              <div className="h-14 w-14 rounded-xl bg-gradient-to-br from-[#1e4e8c] to-[#13233a] flex items-center justify-center text-white font-black text-sm px-3 py-2 shadow-md border border-white/10 shrink-0">
                UBB
              </div>
            </div>
            <div className="text-left min-w-0">
