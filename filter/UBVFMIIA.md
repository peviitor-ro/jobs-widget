# Discipline Informatică Aplicată – Universitatea Transilvania din Brașov

Fișe de disciplină pentru programul de licență **Informatică Aplicată** (UBV FMIIA).

---

## Anul I

### Analiză matematică

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Maria Dimitriu |
| Titular seminar/laborator | proiect |
| Anul | 1 |
| Semestrul | 1 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 5 |

**Competențe profesionale:**

- R.Î.4.1. Absolventul poate să verifice corespondența dintre capacitățile, corectitudinea și eficienta unui algoritm
- sau a unui sistem intenționat și anumite specificații formale.
- R.Î.4.2. Absolventul poate să utilizeze modele formale și teorii matematice pentru a rezolva probleme din diverse

---

### Fundamentele Algebrice ale Informaticii

| Proprietate | Valoare |
|---|---|
| Titular curs | Lector dr. Adelina-Loredana MANEA |
| Titular seminar/laborator | proiect |
| Anul | 1 |
| Semestrul | 1 |
| Evaluare | Colocviu |
| Credite | 5 |
| Ore/săptămână | 4 |

---

### A L GO R I TM I  F UNDA M EN T AL I

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. Dr. Alexandra Băicoianu |
| Titular seminar/laborator | proiect |
| Anul | I |
| Semestrul | I |
| Evaluare | Examen |
| Ore/săptămână | 5 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Fundamentele programării

| Proprietate | Valoare |
|---|---|
| Titular curs | Prof. Dr. Deaconu Adrian |
| Anul | 1 |
| Semestrul | 1 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Tematică curs:**

- Noțiuni introductive de programare (cod sursă, cod obiect, compilare,
- Structura unui program în C: includeri, antete de funcții și rolul lor,
- Macrocomenzi.
- Tipuri numerice de date. Operatorii limbajului C
- Funcții de scriere și citire in C
- Instrucțiuni de decizie. Instrucțiuni repetitive
- Tipul char
- Pointeri. Tablouri. Alocare dinamică.

---

### Logică matematică și computațională

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Laura Ciupală |
| Titular seminar/laborator | proiect |
| Anul | 1 |
| Semestrul | 1 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 3 |

**Competențe profesionale:**

- R.Î.4.1. Absolventul poate să verifice corespondența dintre capacitățile, corectitudinea și eficienta unui
- algoritm sau a unui sistem intenționat și anumite specificații formale.
- R.Î.4.2. Absolventul poate să utilizeze modele formale și teorii matematice pentru a rezolva probleme din

---

### Etică și integritate academică I

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. Dr. SASU Adela |
| Titular seminar/laborator | proiect |
| Anul | 1 |
| Semestrul | 1 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 3 |

**Competențe profesionale:**

- R.Î.3.2. Absolventul poate evalua și selecta instrumente informatice adecvate pentru rezolvarea problemelor
- interdisciplinare, având abilitatea de a analiza nevoile specifice ale unui proiect și de a alege soluții tehnologice
- R.Î.3.4. Studentul/absolventul dezvoltă interesul pentru literatura de specialitate națională și internațională,

---

### Educație fizică şi sport 1/2

| Proprietate | Valoare |
|---|---|
| Titular curs | - |
| Titular seminar/laborator | proiect |
| Anul | 1 |
| Semestrul | 2 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 1 |

---

### Limba Engleza 1

| Proprietate | Valoare |
|---|---|
| Titular curs | Lector dr. Anamaria Schwab |
| Titular seminar/laborator | proiect |
| Anul | I |
| Semestrul | 1 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 2 |

**Tematică curs:**

- 
- Knowledge (continued). Reading to
- Organisation. Difference between
- 
- 
- 
- Growth. Using sources. Referring to

---

### Limba germană 1-2

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Claudia Gabriela ȘERBU |
| Titular seminar/laborator | proiect |
| Anul | I |
| Semestrul | I |
| Credite | 5 |
| Ore/săptămână | 2 |

**Tematică curs:**

- Fachwortschatz zum Thema „Lebenslauf“,
- Fachwortschatz zum Thema: „Meine
- Fachwortschatz: „Briefe und E-mails“,
- Fachwortschatz zum Thema: „Studium
- Fachwortschatz zum Thema „Telefonate
- Fachwortschatz zum Thema „Arbeit und
- Fachwortschatz zum Thema „Sitzungen,

---

### N O Ț I UN I  F UNDA M E NTA LE  DE  I N FO R MAT I C Ă

| Proprietate | Valoare |
|---|---|
| Titular curs | 2.3 Titularul activităților de seminar/ laborator/ |
| Titular seminar/laborator | proiect |
| Anul | I |
| Semestrul | I |
| Evaluare | Verificare |
| Ore/săptămână | 2 |

**Competențe profesionale:**

- R.Î.2.1. Absolventul poate să parcurgă ciclul de viață al unui sistem informatic, începând cu ancheta preliminară și
- terminând cu exploatarea și menținerea sistemului.
- R.Î.2.3. Absolventul poate să proiecteze soluții software eficiente și scalabile pentru fiecare fază a ciclului de viață.

---

### N O Ț I UN I   F UNDA M ENT AL E  D E  M AT EMA T I CĂ

| Proprietate | Valoare |
|---|---|
| Titular curs | 2.3 Titularul activităților de seminar/ laborator/ |
| Titular seminar/laborator | proiect |
| Anul | I |
| Semestrul | I |
| Evaluare | Verificare |
| Ore/săptămână | 2 |

**Competențe profesionale:**

- R.Î.4.2. Absolventul poate să utilizeze modele formale și teorii matematice pentru a rezolva probleme din diverse
- domenii ale informaticii.
- R.Î.4.3. Absolventul poate să utilizeze concepte și tehnici matematice pentru a analiza și proiecta algoritmi și

---

### Algebră liniară, geometrie analitică și diferențială

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Stan Ion-Gabriel |
| Titular seminar/laborator | proiect |
| Anul | 1 |
| Semestrul | 2 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.4.1. Absolventul poate să verifice corespondența dintre capacitățile, corectitudinea și eficienta unui algoritm
- sau a unui sistem intenționat și anumite specificații formale.
- R.Î.4.2. Absolventul poate să utilizeze modele formale și teorii matematice pentru a rezolva probleme din diverse

**Tematică curs:**

- Pseudoinverse;
- Spatii euclidiene
- Transformări liniare
- Vectori proprii și valori proprii
- Forme biliniare și forme pătratice
- Spațiul vectorilor liberi
- Planul și dreapta în spațiu
- Conice

---

### ARHITECTURA SISTEMELOR DE CALCUL

| Proprietate | Valoare |
|---|---|
| Titular curs | ANCA VASILESCU |
| Titular seminar/laborator | LIANA LUMINIȚA BOCA |
| Anul | 1 |
| Semestrul | 2 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.2.4. Absolventul poate să implementeze și să testeze componente software conform specificațiilor
- funcționale și non-funcționale.
- R.Î.2.8. Absolventul poate să lucreze în echipă pentru a parcurge ciclul de viață al unui sistem informatic.

---

### Programare orientată pe obiecte

| Proprietate | Valoare |
|---|---|
| Titular curs | Prof. Dr. Deaconu Adrian |
| Anul | 1 |
| Semestrul | 2 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

---

### Structuri de date

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Ioana Cristina Plajer |
| Titular seminar/laborator | Lect. dr. Ioana Cristina Plajer |
| Anul | 1 |
| Semestrul | 2 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

**Tematică curs:**

- Introducere structuri de date.
- Structuri de date elementare:
- 1 Liste înlănțuite
- 2 Stive și cozi
- 3. Exemple practice
- Tabele de dispersie. Aplicații.
- Arbori oarecare: definiții,
- Heap-uri binare. Coadă de priorități.

---

### Sisteme de Operare

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Honorius Gâlmeanu |
| Titular seminar/laborator | laborator/proiect |
| Anul | 1 |
| Semestrul | 2 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.3. Absolventul poate să utilizeze biblioteci și framework-uri pentru îmbunătățirea performanțelor și
- funcționalitatea aplicațiilor software.

**Tematică curs:**

- Scopul sistemelor de operare. Organizarea și arhitectura unui
- Structurile sistemului de operare. Servicii. Interfața cu
- Sisteme de fișiere. Conceptul de fișier. Metode de acces.
- Managementul proceselor. Procese. Planificarea proceselor.
- Thread-uri și concurență. Programare multi-core. Modele
- Planificarea CPU-ului. Concepte. Algoritmi de planificare:
- Primitive pentru sincronizarea proceselor. Condiții de
- Probleme de sincronizare. Probleme clasice de

---

### Educație fizică şi sport 1/2

| Proprietate | Valoare |
|---|---|
| Titular curs | - |
| Titular seminar/laborator | proiect |
| Anul | 1 |
| Semestrul | 2 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 1 |

---

### Limba germană 1-2

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Claudia Gabriela ȘERBU |
| Titular seminar/laborator | proiect |
| Anul | I |
| Semestrul | I |
| Credite | 5 |
| Ore/săptămână | 2 |

**Tematică curs:**

- Fachwortschatz zum Thema „Lebenslauf“,
- Fachwortschatz zum Thema: „Meine
- Fachwortschatz: „Briefe und E-mails“,
- Fachwortschatz zum Thema: „Studium
- Fachwortschatz zum Thema „Telefonate
- Fachwortschatz zum Thema „Arbeit und
- Fachwortschatz zum Thema „Sitzungen,

---

### Limba Engleză 2

| Proprietate | Valoare |
|---|---|
| Titular curs | Lector dr. Anamaria Schwab |
| Titular seminar/laborator | proiect |
| Anul | I |
| Semestrul | 2 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 2 |

**Tematică curs:**

- 
- 
- 
- 
- 
- Consumption (continued). Writing:
- Energy. Academic focus: problem

---

## Anul II

### Algoritmica grafurilor

| Proprietate | Valoare |
|---|---|
| Titular curs | Prof. Dr. Adrian Marius Deaconu |
| Anul | 2 |
| Semestrul | 1 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Tematică curs:**

- Noțiuni introductive legate de teoria grafurilor
- Parcurgere parțială, parcurgere totală a grafurilor, sortare
- Componente conexe, componente tare conexe
- Cicluri și arbori
- Arbori parțiali minimi
- Arborescențe
- Distanțe și drumuri minime
- Rețele. Flux într-o rețea. Tăietură minimă.. Rețea reziduală.

---

### Limbaje formale și compilatoare

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Alexandra Băicoianu |
| Titular seminar/laborator | laborator/proiect |
| Anul | 2 |
| Semestrul | 3 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare,
- inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

**Tematică curs:**

- Introducere în domeniu, structura
- Sisteme de rescriere.
- 1 Algoritmi normali în sens Markov.
- 2 Limbaje. Gramatici. Ierarhia Chomsky
- Limbaje regulate. Automate finite.
- 1. Noțiuni generale
- 2. Legătura dintre automate finite și
- 3. Minimizarea automatelor finite

---

### Medii şi instrumente de programare

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. Dr. Răzvan Bocu |
| Anul | 2 |
| Semestrul | 3 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare,
- inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Baze de date

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Aldea Constantin Lucian |
| Titular seminar/laborator | laborator/proiect |
| Anul | 2 |
| Semestrul | 3 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.3.1. Absolventul poate utiliza instrumente informatice (folosind tehnologii specifice) pentru a colecta,
- analiza și interpreta date din diferite domenii de activitate.
- R.Î.3.2. Absolventul poate evalua și selecta instrumente informatice adecvate pentru rezolvarea problemelor

---

### C ++  M od er n  ap l i ca t  în  I n te l ig en ț ă  Ar t if i c ial ă

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. Dr. Alexandra Băicoianu |
| Titular seminar/laborator | proiect |
| Anul | 2 |
| Semestrul | 3 |
| Evaluare | Examen |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare,
- inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Educație fizică şi sport 1/2

| Proprietate | Valoare |
|---|---|
| Titular curs | - |
| Titular seminar/laborator | proiect |
| Anul | 1 |
| Semestrul | 2 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 1 |

---

### Sisteme moderne de modelare grafică 3D

| Proprietate | Valoare |
|---|---|
| Anul | 2 |
| Semestrul | 5 |
| Evaluare | Verificare |
| Credite | 4 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Limbaje formale și compilatoare

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Majercsik Luciana |
| Titular seminar/laborator | laborator/proiect |
| Anul | 2 |
| Semestrul | 3 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare,
- inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### P R O G R AMA R E  D I S TR I B UI T Ă

| Proprietate | Valoare |
|---|---|
| Titular curs | Prof. Dr. Sabin Tăbîrcă |
| Titular seminar/laborator | proiect |
| Anul | 2 |
| Semestrul | 3 |
| Evaluare | Verificare |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.
- R.Î.1.3. Absolventul poate să utilizeze biblioteci și framework-uri pentru îmbunătățirea performanțelor și

---

### Securitate IT

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Aldea Constantin Lucian |
| Titular seminar/laborator | laborator/proiect |
| Anul | 2 |
| Semestrul | 3 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.2.1. Absolventul poate să parcurgă ciclul de viață al unui sistem informatic, începând cu ancheta preliminară
- și terminând cu exploatarea și menținerea sistemului.
- R.Î.2.6. Absolventul poate să aplice măsuri de securitate pentru protejarea datelor pe tot parcursul ciclului de

---

### Limba Engleză 3

| Proprietate | Valoare |
|---|---|
| Titular curs | Lector dr. Anamaria Schwab |
| Titular seminar/laborator | proiect |
| Anul | II |
| Semestrul | 3 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 2 |

**Tematică curs:**

- Power – Academic focus: reporting
- Power (continued) – Conversation.
- 
- Progress (continued) – Stance in
- 
- Nature (continued) – Paragraphs:
- Internet Crime. Academic focus:

---

### Limba germană 3-4

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Claudia Gabriela ȘERBU |
| Titular seminar/laborator | proiect |
| Anul | II |
| Semestrul | III |
| Credite | 5 |
| Ore/săptămână | 2 |

**Tematică curs:**

- Fachwortschatz: „Im Ausland arbeiten”,
- Fachwortschatz: „Arbeitsbedingungen
- Fachwortschatz: „Netzwerke der
- Fachwortschatz: „Umwelt und
- Fachwortschatz: Übersetzungen von
- Fachwortschatz: Übersetzungen von
- Fachwortschatz: Dolmetscher auf

---

### Inteligență artificială

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Sasu Lucian Mircea |
| Titular seminar/laborator | proiect |
| Anul | 2 |
| Semestrul | 4 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare,
- inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Medii vizuale de programare

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Sasu Lucian Mircea |
| Titular seminar/laborator | proiect |
| Anul | 2 |
| Semestrul | 4 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare,
- inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Rețele de calculatoare

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr.  Nănău Corina Ştefania |
| Titular seminar/laborator | proiect |
| Anul | 2 |
| Semestrul | 4 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- C1. Programarea în limbaje de nivel înalt
- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv

---

### Probabilități și statistică matematică

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Andreea Fulga |
| Titular seminar/laborator | laborator/proiect |
| Anul | 2 |
| Semestrul | 4 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.3.1. Absolventul poate utiliza instrumente informatice (folosind tehnologii specifice) pentru a colecta,
- analiza și interpreta date din diferite domenii de activitate.
- R.Î.3.2. Absolventul poate evalua și selecta instrumente informatice adecvate pentru rezolvarea problemelor

---

### Inferență Statistică în Machine Learning

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Honorius Gâlmeanu |
| Titular seminar/laborator | laborator/proiect |
| Anul | 2 |
| Semestrul | 1 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare,
- inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

**Tematică curs:**

- Introducere. Probabilitate. Probabilităţi condiţionate.
- Valori aşteptate. Variabilitate. Medie și deviație standard.
- Distribuţii importante. Percentile în distribuția gaussiană.
- Distribuția Student-T. Intervale de confidenţă T. A/B testing.
- Power. Legătura cu Type-II error. Calculul power. Testarea
- Corelaţia şi regresia liniară. Metoda celor mai mici pătrate.
- Modelul regresiei liniare. Coeficienții regresiei. Reziduuri.
- Proprietăţile regresiei multi-variabilă. Heatmap pentru corelații.

---

### Educație fizică şi sport 1/2

| Proprietate | Valoare |
|---|---|
| Titular curs | - |
| Titular seminar/laborator | proiect |
| Anul | 1 |
| Semestrul | 2 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 1 |

---

### Dezvoltarea aplicațiilor în cloud

| Proprietate | Valoare |
|---|---|
| Titular curs | Andrei Ghiurțu (Tremend, Publicis Sapient) |
| Titular seminar/laborator | proiect |
| Anul | 2 |
| Semestrul | 4 |
| Evaluare | Verificare |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.4. Absolventul poate să dezvolte aplicații complexe cu cerințele propuse de către utilizatori.
- Responsabilitate și autonomie

---

### Ș A BL OA N E  D E  P RO I EC T A RE

| Proprietate | Valoare |
|---|---|
| Titular curs | Mihai-Ștefan Iolu (Magic Lane) |
| Titular seminar/laborator | proiect |
| Anul | 2 |
| Semestrul | 4 |
| Evaluare | Verificare |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Modelarea și simularea sistemelor dinamice

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Chirilă Adina |
| Titular seminar/laborator | proiect |
| Anul | 2 |
| Semestrul | 2 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.4. Absolventul poate să dezvolte aplicații complexe cu cerințele propuse de către utilizatori.
- Responsabilitate și autonomie

---

### T eh n i ci  d e  t es t ar e  sof t wa r e

| Proprietate | Valoare |
|---|---|
| Titular curs | Ungureanu Andrei-Silviu (Siemens Industry Software) |
| Titular seminar/laborator | proiect |
| Anul | 3 |
| Semestrul | 6 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- ➢ R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- ➢ R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de
- programare, inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și

---

### Dezvoltarea aplicațiilor Web și Mobile cu Angular, .Net și Android

| Proprietate | Valoare |
|---|---|
| Titular curs | Gabriel Alexandru (Cornerstone) |
| Titular seminar/laborator | proiect |
| Anul | 3 |
| Semestrul | 2 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Tematică curs:**

- Noțiuni introductive despre comunicarea server-
- Noțiuni generale de securitate si autentificare.
- Noțiuni introductive despre sisteme de versionare
- Noțiuni avansate de backend.
- Noțiuni avansate client - frontend.
- Noțiuni avansate client - mobile.
- Prezentarea unor softuri utilitare în gestionarea
- Sesiune de lucru în echipa inter-modul în vederea

---

### Limba Engleză 4

| Proprietate | Valoare |
|---|---|
| Titular curs | Lector dr. Anamaria Schwab |
| Titular seminar/laborator | proiect |
| Anul | II |
| Semestrul | 4 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 2 |

**Tematică curs:**

- Innovation and Invention. Reading:
- Innovation and Invention (continued).
- Ways
- 
- Communicating Science. Reading:
- Communicating Science (continued).
- IT in Education and Business.

---

### Limba germană 3-4

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Claudia Gabriela ȘERBU |
| Titular seminar/laborator | proiect |
| Anul | II |
| Semestrul | III |
| Credite | 5 |
| Ore/săptămână | 2 |

**Tematică curs:**

- Fachwortschatz: „Im Ausland arbeiten”,
- Fachwortschatz: „Arbeitsbedingungen
- Fachwortschatz: „Netzwerke der
- Fachwortschatz: „Umwelt und
- Fachwortschatz: Übersetzungen von
- Fachwortschatz: Übersetzungen von
- Fachwortschatz: Dolmetscher auf

---

## Anul III

### Verificarea și validarea sistemelor software

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Cârstea Claudia |
| Titular seminar/laborator | laborator/ proiect |
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.2.1. Absolventul poate să parcurgă ciclul de viață al unui sistem informatic, începând cu ancheta preliminară și
- terminând cu exploatarea și menținerea sistemului.
- R.Î.2.2. Absolventul poate să identifice și să evalueze riscurile asociate cu fiecare fază a ciclului de viață.

---

### Tehnologii WEB

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Dumitrescu Silviu |
| Titular seminar/laborator | proiect |
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Dezvoltarea Aplicațiilor Mobile

| Proprietate | Valoare |
|---|---|
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

---

### Practică de specialitate

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect.dr. Nicoleta Enache-David |
| Titular seminar/laborator | laborator/proiect |
| Anul | 3 |
| Semestrul | 1 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 8 |

**Competențe profesionale:**

- R.Î.2.1. Absolventul poate să parcurgă ciclul de viață al unui sistem informatic, începând cu ancheta preliminară
- și terminând cu exploatarea și menținerea sistemului.
- R.Î.2.2. Absolventul poate să identifice și să evalueze riscurile asociate cu fiecare fază a ciclului de viață.

---

### Deep Learning

| Proprietate | Valoare |
|---|---|
| Titular curs | Dr. Musat Bogdan-Adrian |
| Titular seminar/laborator | proiect |
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Verificare |
| Ore/săptămână | 4 |

---

### Procesarea imaginilor digitale

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Plajer Ioana Cristina |
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Colocviu |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

**Tematică curs:**

- Introducere, etapele principale ale procesării imaginilor,
- Îmbunătățirea imaginilor - operații punctuale
- 1 Binarizare
- 2 Modificare luminozitate / contrast
- 3. Histograma
- Segmentare de bază. Thresholding.
- Filtrare în domeniul spațial:
- 1 Reducerea zgomotului - filtrare trece-jos (filtrul medie

---

### Procese stohastice și aplicații

| Proprietate | Valoare |
|---|---|
| Titular curs | Prof. Dr. Mihai PASCU |
| Titular seminar/laborator | Prof. Dr. Mihai PASCU |
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.4.2. Absolventul poate să utilizeze modele formale și teorii matematice pentru a rezolva probleme din
- domenii ale informaticii.
- R.Î.4.3. Absolventul poate să utilizeze concepte și tehnici matematice pentru a analiza și proiecta algoritmi și

**Tematică curs:**

- Spatiu de probabilitate (sigma-algebră,
- Aşteptare conditionată în raport cu o
- Martingale (martingale / submartingale
- Teorema Doob a timpului optional de
- Transformări
- Inegalităti şi teoreme de convergentă
- Mişcarea
- Integrala

---

### Introducere în calcul cuantic

| Proprietate | Valoare |
|---|---|
| Titular curs | Sorana Catrina (Harman international industries) |
| Titular seminar/laborator | laborator/proiect |
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare,
- inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Inginerie soft

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Cârstea Claudia |
| Titular seminar/laborator | proiect |
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.2.1. Absolventul poate să parcurgă ciclul de viață al unui sistem informatic, începând cu ancheta preliminară
- și terminând cu exploatarea și menținerea sistemului.
- R.Î.2.2. Absolventul poate să identifice și să evalueze riscurile asociate cu fiecare fază a ciclului de viață.

**Tematică curs:**

- Elemente de teoria generală a sistemelor din
- Introducere în ingineria sistemelor soft: motivație,
- Sisteme informaționale. Fluxuri informaționale.
- Limbajul de modelare UML
- Tipuri de diagrame UML
- Diagrame de stare
- Prezentarea  tool-lui Visual Paradigm for UML
- Diagrame de comunicare

---

### Dezvoltarea jocurilor pe calculator cu Unity

| Proprietate | Valoare |
|---|---|
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Verificare |
| Credite | 4 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Procesarea imaginilor cu ajutorul algoritmilor inteligenți

| Proprietate | Valoare |
|---|---|
| Titular curs | Lect. dr. Plajer Ioana Cristina |
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

**Tematică curs:**

- Introducere în procesarea de imagini, reprezentarea
- Algoritmi de bază
- 1 Histograma. Binarizare
- 2 Filtrare trece-jos și trece-sus. Elemente de bază și
- Introducere în morfologie matematică.
- Algoritmi de codificare și compresie.
- Algoritmi evolutivi în procesarea imaginilor.
- Analiza și procesarea imaginilor bazată pe rețele neuronale.

---

### Grafică pe calculator

| Proprietate | Valoare |
|---|---|
| Titular curs | Prof. Dr. Deaconu Adrian |
| Anul | 1 |
| Semestrul | 2 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

---

### Programare logică și funcțională

| Proprietate | Valoare |
|---|---|
| Titular curs | Lector dr. Adelina Manea |
| Titular seminar/laborator | proiect |
| Anul | III |
| Semestrul | II |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î. 4.1. Absolventul poate să utilizeze modele formale și teorii matematice pentru a rezolva probleme din
- diverse domenii ale informaticii.
- R.Î. 4.2. Absolventul poate sa utilizeze concepte și tehnici matematice pentru a analiza și proiecta algoritmi

---

### E LA B O RA R EA  L UC R Ă RI I  D E  L I CE N ȚĂ

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. Dr. Alexandra Băicoianu – coordonator program studii (și |
| Anul | 3 |
| Semestrul | 6 |
| Evaluare | Verificare |
| Ore/săptămână | 6 |

**Competențe profesionale:**

- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.
- R.Î.1.3. Absolventul poate să utilizeze biblioteci și framework-uri pentru îmbunătățirea performanțelor și

---

### Sisteme de Gestiune a Bazelor de Date

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Honorius Gâlmeanu |
| Titular seminar/laborator | laborator/proiect |
| Anul | 3 |
| Semestrul | 2 |
| Evaluare | Examen |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare,
- inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

**Tematică curs:**

- Proprietățile DBMS-urilor. Concepte cheie. Roluri. Modelul
- SQL: select, relații și operatori, subquery-uri în clauza WHERE.
- SQL: operatori join, agregare, valori NULL, modificarea datelor
- Dependințe funcționale, forma BCNF
- Dependințe cu valori multiple și forma 4NF. Contraexemple.
- Descrierea modelelor relaționale cu UML
- Indecși. Tranzacții. Proprietățile tranzacțiilor. Nivele de
- Constrângeri. Integritate referențială. Triggers.

---

### Antreprenoriat și strategii de afaceri

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. Dr. Dana Adriana Lupşa-Tătaru |
| Titular seminar/laborator | proiect |
| Anul | 3 |
| Semestrul | 6 |
| Evaluare | Verificare |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.3.2. Absolventul poate evalua și selecta instrumente informatice adecvate pentru rezolvarea problemelor
- interdisciplinare, având abilitatea de a analiza nevoile specifice ale unui proiect și de a alege soluții tehnologice
- R.Î.3.4. Studentul/absolventul dezvoltă interesul pentru literatura de specialitate națională și internațională,

---

### Full Stack development of web - based ML applications

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. Dr. Alexandra Băicoianu |
| Titular seminar/laborator | proiect |
| Anul | 3 |
| Semestrul | 6 |
| Evaluare | Verificare |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.2.1. Absolventul poate să parcurgă ciclul de viață al unui sistem informatic, începând cu ancheta preliminară și
- terminând cu exploatarea și menținerea sistemului.
- R.Î.2.3. Absolventul poate să proiecteze soluții software eficiente și scalabile pentru fiecare fază a ciclului de

---

### Limbajul Natural în Machine Learning

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Honorius Gâlmeanu |
| Titular seminar/laborator | proiect |
| Anul | 3 |
| Semestrul | 6 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare,
- inclusiv programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

**Tematică curs:**

- Introducere în modelele lingvistice. Aplicații ale LLM.
- Seturi de date mari. Arhitectura GPT.
- Tokens și embeddings. Crearea codificării contextualizate a
- Algoritmul Word2Vec. Contrastive Training. Encodarea
- Rețele neuronale recurente. Modele de tip GRU și LSTM.
- Mecanismul attention. Modelarea secvențelor lungi.
- Implementarea self-attention. Causal attention mask.
- Codarea unei arhitecturi LLM. Layer normalization pentru

---

### Procesarea inteligentă a volumelor mari de informații vizuale

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Alexandra Băicoianu |
| Anul | 3 |
| Semestrul | 5 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

**Tematică curs:**

- Introducere în procesarea de imagini, reprezentarea
- Noțiuni fundamentale de Big Data și Cloud Computing
- Ințelegerea informației vizuale prin Deep Learning
- Procesarea
- Procesarea imaginilor medicale la scară mare
- Arhitecturi scalabile pentru procesarea informației vizuale

---

### PROGRAMARE PARALELĂ

| Proprietate | Valoare |
|---|---|
| Titular curs | Prof. Dr. Sabin Tăbîrcă |
| Titular seminar/laborator | Drd. Bogdan Mihai Petre |
| Anul | 3 |
| Semestrul | 2 |
| Evaluare | Verificare |
| Credite | 4 |
| Ore/săptămână | 4 |

---

### Arhitectura aplicațiilor front-end folosind Angular

| Proprietate | Valoare |
|---|---|
| Anul | 3 |
| Semestrul | 6 |
| Evaluare | Verificare |
| Credite | 4 |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

### Dezvoltarea aplicațiilor Web

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. dr. Alexandra Băicoianu |
| Titular seminar/laborator | proiect |
| Anul | 3 |
| Semestrul | 2 |
| Evaluare | Verificare |
| Credite | 5 |
| Ore/săptămână | 4 |

---

### M O DE LA R E Ș I  O R CH ES T RA R E  C U L L M - UR I  Ș I  A G E NȚ I  A I

| Proprietate | Valoare |
|---|---|
| Titular curs | Conf. Dr. Alexandra Băicoianu |
| Titular seminar/laborator | proiect |
| Semestrul | II |
| Evaluare | Verificare |
| Ore/săptămână | 4 |

**Competențe profesionale:**

- R.Î.1.1. Absolventul poate să scrie cod clar și bine structurat în limbaje de programare specifice.
- R.Î.1.2. Studentul/absolventul alege, descrie, analizează și explică paradigmele moderne de programare, inclusiv
- programarea funcțională, orientată pe obiect și paralelă, utilizând limbaje și framework-uri actuale.

---

